"""QLoRA supervised fine-tuning of Qwen2.5-VL-3B for birth-certificate extraction.

Manual training loop (kept transparent on purpose): for each labeled certificate we
build the exact inference prompt, append the gold JSON as the assistant answer, mask
the prompt tokens, and train a 4-bit LoRA so the model learns this template's
box-to-field mapping and digit habits. Designed to run on Modal (see
birthcert_finetune_modal.py) but importable/runnable anywhere with a GPU.
"""
...
# LoRA on the language-model projections (vision tower stays frozen — far more
# stable when the labeled set is small).
...
def train(
    ...
    *,
    ...
    if not train_rows:
        raise SystemExit(f"No training rows in {data_dir/'train.jsonl'}. Label some certificates first.")
    ...
    def build_example(row: dict[str, Any]):
        """Return (inputs, labels) with the prompt portion masked to -100."""
    ...
    for epoch in range(epochs):
        ...
        if skip_pct > 5.0 and epoch == 0:
            print(f"[ft] WARNING: >{skip_pct:.0f}% of examples exceed max_seq_len={max_seq_len}. Consider increasing max_seq_len to 5120.", flush=True)
        if save_every_epochs and (epoch + 1) % save_every_epochs == 0:
            model.save_pretrained(str(output_dir / f"epoch{epoch+1}"))
