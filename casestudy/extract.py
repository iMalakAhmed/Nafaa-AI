"""Core case-study extractor: Qwen2.5-VL -> validated JSON record."""
...
class CaseStudyExtractor:
    def __init__(
        ...
        *,
        ...
        print(f"[casestudy] loading {model_name} ({torch_dtype}, attn={attn_implementation}) ...", flush=True)
        ...
        if adapter_path:
            ...
            print(f"[casestudy] attaching LoRA adapter: {adapter_path}", flush=True)
        ...
        *,
        ...
        try:
            ...
            if _looks_degenerate(raw_text):
                print(f"[casestudy] {doc_id}: degenerate output, retrying ...", flush=True)
...
def run_batch(
    ...
    *,
    ...
    for image_path in image_paths:
        ...
        print(f"[casestudy] wrote {out_file}", flush=True)
        if commit_each:
            commit_each()
    return written
